...
def run_baseline(protocol, records, latency_model):
    ...
    for rec in records:
        ...
        req = Request(request_id=rec.trace_id, kind=RequestKind.REAL, op=rec.op, address=BlockAddress(logical_id=rec.logical_id), data=b'\x00'*4096 if rec.op == OperationType.WRITE else None, arrival_time=rec.timestamp, issued_time=start_service, tag="baseline")
