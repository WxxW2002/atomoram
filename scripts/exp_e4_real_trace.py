...
def run_baseline(protocol, records, latency_model, block_size):
    current_time, latencies = 0.0, []
    for rec in records:
        start_service = max(current_time, rec.timestamp)
        queueing_delay = start_service - rec.timestamp
        req = Request(request_id=rec.trace_id, kind=RequestKind.REAL, op=rec.op, address=BlockAddress(logical_id=rec.logical_id), data=b'\x00'*block_size if rec.op == OperationType.WRITE else None, arrival_time=rec.timestamp, issued_time=start_service, tag="baseline")
...
def run_e4():
    ...
    for name, path in traces.items():
        ...
        path_lats = run_baseline(PathORAM(cfg.storage), records, latency_model, block_size)
        ring_lats = run_baseline(RingORAM(cfg.storage), records, latency_model, block_size)
